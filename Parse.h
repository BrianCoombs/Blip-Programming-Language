...
enum TokenType {
	...
	NAME, // 3
	STRING,  // not currently used or supported by Parse.h/Input.cpp 4
	END, // special marker for end of input 5
	INVALID // this token type should never appear and represents a parsing error 6
};
 ...
 * when parsing a // comment, you must ignore any token or token that remain on the current
 * line. The skip_line is provided for this task
 */
void skip_line(void);

/* set_input allows you to open a text file to read token from. By default, read_next_token
 * reads token from the standard input (e.g., the keyboard). The argument provided to set_input
